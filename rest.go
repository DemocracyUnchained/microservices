...
func StatesShow(w http.ResponseWriter, r *http.Request) {
  ...
  for rows.Next() {
    ...
    for powerful_rows.Next() {
      ...
      checkErr(err)
      state_report.Vs.PercentAsPowerful = (float32(state_report.Vs.VotingEligiblePopulation)/float32(state_report.Vs.Votes)) / (float32(state_report.Voter.VotingEligiblePopulation) / float32(state_report.ElectoralVote.Votes))
    }
